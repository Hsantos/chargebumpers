...
public class CarBehaviour : MonoBehaviourPun
{
	...
	[SerializeField]
	private CustomCarPowerup carPowerup;
	...
	private void Update ()
	{
		...
		direction = Input.GetAxis ("Horizontal") + inputHorizontal;
		...
		verticalDirection = Input.GetAxis("Vertical") + inputVertical;

		if (Mathf.Abs (verticalDirection) < 0.1f) {
			motorTorque = 0.0f;
			brakeTorque = Mathf.Lerp (brakeTorque, rb.mass, Time.deltaTime * 2.0f);
		} else {
			motorTorque = Mathf.Lerp (motorTorque, verticalDirection * rb.mass * torqueForceWheel * carBalance.Speed, Time.deltaTime);
			brakeTorque = 0.0f;
		}
	...
	void StabilizeVehicle(){
		float forceFrontLeft = carBalance.Weight;
		float forceFrontRight = carBalance.Weight;
		float forceReadLeft = carBalance.Weight;
		float forceReadRight = carBalance.Weight;
		WheelHit hit;
